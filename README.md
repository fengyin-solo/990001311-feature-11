...
- **批量审核**：多选待审留言批量通过/拒绝，支持统一处理意见、提交前预览与逐条结果反馈
- **响应式布局**：适配手机和电脑端
...
4. 删除不当留言
5. 查看留言详情（含处理意见与审核时间）
6. 批量审核：勾选多条待审核留言，点击"批量通过/批量拒绝"，预览确认受影响条目并填写统一处理意见后提交；结果逐条反馈，状态已变化的条目自动跳过且不影响其他条目

## 数据库升级

已有环境启用批量审核功能前，请执行迁移脚本为留言表添加审核字段：

```bash
mysql -u root -p community_board < database/migration_add_audit_fields.sql
```

新安装环境无需执行，安装脚本已包含完整表结构。
